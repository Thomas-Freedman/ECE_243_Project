...
from .advanced_models import StreamingTransformerDecoder, HybridPOSSMDecoder, POYODecoder
...
@dataclass
class HybridArgs:
    """Arguments for POSSM-style hybrid Transformer+SSM model training."""
    outputDir: str
    datasetPath: str
    batchSize: int
    nBatch: int
...
def train_hybrid_model(args: Dict):
    """Train POSSM-style hybrid Transformer+SSM model."""
    parsed = HybridArgs(**args)
    os.makedirs(parsed.outputDir, exist_ok=True)
    _seed_all(parsed.seed)
    device = "cuda" if torch.cuda.is_available() else "cpu"

    with open(os.path.join(parsed.outputDir, "hybrid_args.pkl"), "wb") as f:
        pickle.dump(args, f)
    ...
    model = HybridPOSSMDecoder(
        neural_dim=parsed.nInputFeatures,
        n_phonemes=parsed.nClasses,
        d_model=parsed.modelDim,
        nhead=parsed.modelHeads,
        num_transformer_layers=parsed.numTransformerLayers,
        num_ssm_layers=parsed.numSSMLayers,
        dropout=parsed.dropout,
        stride_len=parsed.strideLen,
        kernel_len=parsed.kernelLen,
        gaussian_smooth_width=parsed.gaussianSmoothWidth,
        ...
        rel_bias_by_head=parsed.relBiasByHead,
        ff_mult=parsed.ffMult,
        ssm_d_state=parsed.ssmDState,
        ssm_d_conv=parsed.ssmDConv,
    ).to(device)
    ...
    metrics = {"valLoss": [], "valCER": []}
    start = time.time()
    print(f"[hybrid] Training POSSM-style hybrid model with {parsed.numTransformerLayers} Transformer + {parsed.numSSMLayers} SSM layers")
    ...
    for step in range(parsed.nBatch):
        ...
        if step % 100 == 0:
            ...
            print(
                f"[hybrid] step={step} val_ctc={val_loss:.4f} cer={cer:.4f}{adapt_msg} time/100={elapsed:.2f}s"
            )
            start = time.time()
            metrics["valLoss"].append(val_loss)
            metrics["valCER"].append(cer)
            with open(os.path.join(parsed.outputDir, "hybrid_stats.pkl"), "wb") as f:
                pickle.dump(metrics, f)

            if best_cer is None or cer < best_cer:
                best_cer = cer
                torch.save(
                    model.state_dict(), os.path.join(parsed.outputDir, "hybrid_weights.pt")
                )
...
def _evaluate_hybrid(
    ...
    """Evaluate hybrid model, same logic as _evaluate for advanced model."""
    model.eval()
    losses = []
    total_edit = 0
    total_len = 0
...
@dataclass
class POYOArgs:
    """Arguments for POYO-style model with spike tokenization and session embeddings."""
    outputDir: str
    datasetPath: str
    batchSize: int
    nBatch: int
...
def train_poyo_model(args: Dict):
    """Train POYO-style model with spike tokenization and session embeddings."""
    parsed = POYOArgs(**args)
    os.makedirs(parsed.outputDir, exist_ok=True)
    _seed_all(parsed.seed)
    device = "cuda" if torch.cuda.is_available() else "cpu"

    with open(os.path.join(parsed.outputDir, "poyo_args.pkl"), "wb") as f:
        pickle.dump(args, f)
    ...
    # Use number of days as sessions if multi-session
    n_sessions = max(parsed.nSessions, len(payload["train"]))
    ...
    model = POYODecoder(
        ...
        n_tokens=parsed.nTokens,
        bin_size=parsed.binSize,
        stride_len=parsed.strideLen,
        kernel_len=parsed.kernelLen,
        gaussian_smooth_width=parsed.gaussianSmoothWidth,
        intermediate_layer=parsed.intermediateLayer,
    ...
    metrics = {"valLoss": [], "valCER": []}
    start = time.time()
    print(f"[poyo] Training POYO model with {parsed.nTokens} tokens and {n_sessions} sessions")
    ...
    for step in range(parsed.nBatch):
        ...
        X, y, X_len, y_len, day_idx = (
            ...
            X_len.to(device),
            y_len.to(device),
            day_idx.to(device),  # Use day_idx as session_idx
        )
        ...
        if step % 100 == 0:
            ...
            print(
                f"[poyo] step={step} val_ctc={val_loss:.4f} cer={cer:.4f}{adapt_msg} time/100={elapsed:.2f}s"
            )
            start = time.time()
            metrics["valLoss"].append(val_loss)
            metrics["valCER"].append(cer)
            with open(os.path.join(parsed.outputDir, "poyo_stats.pkl"), "wb") as f:
                pickle.dump(metrics, f)

            if best_cer is None or cer < best_cer:
                best_cer = cer
                torch.save(
                    model.state_dict(), os.path.join(parsed.outputDir, "poyo_weights.pt")
                )
