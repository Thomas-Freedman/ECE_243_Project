...
class PhonemeLM:
    def __init__(self, lm_path: str, phoneme_map: dict = None):
        if not LM_AVAILABLE:
            raise RuntimeError("kenlm not available. Install kenlm to use phoneme LM.")
...
def beam_search(log_probs, lm_wrapper: PhonemeLM = None, lm_weight=0.8, beam_width=8, blank_id=0, topk_acoustic=5):
    # Running beam search on transcript at each step
    ...
    for idx, (X, y, X_len, y_len, day_idx) in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            for i in range(out['log_probs'].shape[0]):
                ...
                target = y[i][:y_len[i]].cpu().detach().numpy().tolist()
...
def main():
    config = {
        ...
        'seed': 0,
        'use_phoneme_lm': True,
        'phoneme_lm_path': '/content/drive/MyDrive/ECE_243A/phoneme_lm.arpa',
    ...
    dataset_path = '/content/drive/MyDrive/ECE_243A/ptDecoder_ctc'
