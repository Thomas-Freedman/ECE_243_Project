...
print(f"KenLM available: {LM_AVAILABLE}")
print("=" * 70)
...
class PhonemeLM:
    ...
    def _tokens_to_str(tokens):
        # kenlm expects a single string with tokens separated by spaces
        return " ".join(tokens)

    def id_sequence_to_tokens(self, id_seq):
        # id_seq: iterable of ints (phoneme ids). We expect ids mapping to strings.
        tokens = []
        for i in id_seq:
            # i assumed to be python int
...
def beam_search(log_probs, lm_wrapper: PhonemeLM = None,
                                    lm_weight=0.8, beam_width=8, blank_id=0, topk_acoustic=5):
    """
    Simple beam search over time steps using acoustic log_probs (tensor or numpy)
    log_probs: [T, V] log-probabilities (natural log or logit; mixing scales matters).
    Returns best decoded sequence of phoneme ids (without repeated-consecutive collapsing).
    NOTE: This beam search is intentionally lightweight and not optimized for speed.
    """
    ...
    # Each beam: (sequence_list_of_ids, acoustic_score_sum, lm_score_scalar)
    beams = [([], 0.0, 0.0)]
    for t in range(T):
        step = lp[t]  # shape (V,)
        # pick topk acoustic candidates at this time step to limit branching
        ...
        for seq, a_score, l_score in beams:
            for idx in topk_idx:
                ...
                if idx == blank_id:
                    # blank: keep seq unchanged
                    new_seq = tuple(seq)
                    new_a = a_score + token_logp
                    new_l = l_score  # LM doesn't change on blank
                else:
                    # append token
                    new_seq = tuple(list(seq) + [int(idx)])
                    new_a = a_score + token_logp
                    if lm_wrapper is not None:
                        # compute LM score for new sequence (full)
                ...
                # keep highest combined for identical sequences
    ...
    # select best beam by combined score
    best = max(beams, key=lambda b: b[1] + lm_weight * b[2])
    decoded = best[0]
    # collapse repeated consecutive tokens (CTC-style) and remove blanks if any slipped in
    collapsed = []
    prev = None
    for tok in decoded:
        if tok == prev:
...
def evaluate(model, loader, ctc_loss, device, max_batches=20,
             lm_wrapper: PhonemeLM = None, lm_weight: float = 0.8, lm_beam: int = 8):
    model.eval()
    losses = []
    total_edit = 0
    total_len = 0
    ...
    for idx, (X, y, X_len, y_len, day_idx) in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            for i in range(out['log_probs'].shape[0]):
                logits = out['log_probs'][i, :eff_len[i], :]  # [T, V]
                # decode with optional LM (beam search shallow fusion) or greedy argmax
                if lm_wrapper is not None:
                    # run beam search with shallow fusion
                    decoded = beam_search(
                        logits.cpu(),
                        lm_wrapper=lm_wrapper,
                        lm_weight=lm_weight,
                        beam_width=lm_beam,
                        blank_id=0,
                        topk_acoustic=5
                    ...
                    decoded = np.unique(np.array(decoded), return_index=False) if False else decoded
...
def load_phoneme_map(path, n_classes):
    """
    Load a phoneme id->token mapping.
    Accept formats:
      - None -> synthetic PH1...PHN
      - plain text file with one token per line (index starting at 1)
      - pickle dict {int: str}
    """
    if path is None:
...
def main():
    config = {
        ...
        'seed': 0,
        'use_phoneme_lm': True,
        'phoneme_lm_path': 'phoneme_lm.arpa',
        'phoneme_map_path': 'phoneme_map.txt',
        'lm_weight': 0.8,
        'lm_beam': 8,
    }
    ...
    dataset_path = 'neural_seq_decoder/notebooks/ptDecoder_ctc'
    ...
    lm_wrapper = None
    if config['use_phoneme_lm']:
        if not LM_AVAILABLE:
            print("WARNING: use_phoneme_lm=True but kenlm bindings are not installed. LM disabled.")
        else:
            phoneme_map = load_phoneme_map(config['phoneme_map_path'], config['n_classes'])
            try:
                lm_wrapper = PhonemeLM(config['phoneme_lm_path'], phoneme_map)
                print("Phoneme KenLM loaded.")
            except Exception as e:
                print(f"Failed to load KenLM: {e}. Continuing without LM.")
                lm_wrapper = None
    ...
    for step in range(config['n_batch']):
        ...
        if step % 100 == 0:
            val_loss, cer = evaluate(
                model, test_loader, ctc_loss, device,
                lm_wrapper=lm_wrapper,
                lm_weight=config.get('lm_weight', 0.8),
                lm_beam=config.get('lm_beam', 8)
            )
